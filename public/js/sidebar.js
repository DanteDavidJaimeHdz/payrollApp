...
/*!*********************************************!*\
  !*** ./resources/js/lib/menubar/sidebar.js ***!
  \*********************************************/
...
!function ($) {
  ...
  var sidebar = function sidebar(element, options) {
    ...
    this.$html = $('html');
    this.$body = $('body');
    this.$window = $(window); // set in true when first time were clicked on toggle button
  ...
  sidebar.prototype.init = function () {
    var _this = this; // no transition enable


    _this.$body.addClass('sidebar-notransition'); // init Nano Scroller
    ...
    _this.windowResize(); // no transition disable
  ...
  sidebar.prototype.showsidebar = function () {
    ...
    setTimeout(function () {
      ...
      _this.$nano.nanoScroller(); // resize for charts reinit
  ...
  sidebar.prototype.hidesidebar = function () {
    ...
    _this.$body.addClass('sidebar-hide'); // destroy scroller on hidden sidebar
  ...
  sidebar.prototype.closeSub = function (subMenu) {
    ...
    subMenu.css('display', 'block').stop().slideUp(_this.options.duration, 'swing', function () {
      // close child dropdowns
      $(this).find('li a.sidebar-sub-toggle').next().attr('style', ''); // reinit nano scroller
  ...
  sidebar.prototype.openSub = function (subMenu, toggleParent) {
    ...
    subMenu.css('display', 'none').stop().slideDown(_this.options.duration, 'swing', function () {
  ...
  sidebar.prototype.useGestures = function () {
    ...
    _this.$window.on('touchend', function (e) {
      if (touchStart) {
        ...
        touchStart = 0; // swipe min width 100px
  ...
  sidebar.prototype.windowResize = function () {
    var _this = this; // if user currently changed size of sidebar, stop change it
  ...
  $('.sidebar').each(function () {
    var options = $.extend({}, sidebar.DEFAULTS, $(this).data(), (typeof option === "undefined" ? "undefined" : _typeof(option)) == 'object' && option);
